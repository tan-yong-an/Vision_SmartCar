...
void draw_rect_on_slice_buffer(uint16_t* pcam, int srcw, int cury, int stride, odresult_t* podret, int retcnt, int slice_height)
{
    ...
    {
		uint32_t color = 0xff0000ff; // 设置为红色 0xffffffff
        uint16_t* phorline = 0;
...
////		od_result.res_s = (od_result.res_y2 -od_result.res_y1) * (od_result.res_x2 -od_result.res_x1);
//		int16_t adjust_x1 = 0.37 * ( 0.5 * od_result.res_x1 + 0.5 * od_result.res_x2 - 160 );
//        od_result.res_y1 = 0.26 * ( 240 - 0.5 * od_result.res_y1 - 0.5 * od_result.res_y2 );
//        od_result.res_y2 = (int)(odres->score*100);
...
void zf_model_init(void)
{
    ...
    postProcessParams.threshold = 0.5;//默认0.6降低需要增加    置信度过滤：
    postProcessParams.nms = 0.2;//默认0.2
